    ...
    <div v-else class="page">
      ...
      <div class="page-right">
        <header class="mobile-header">
          ...
          <button class="btn-logout-mobile" @click="cerrarSesion">Salir</button>
        </header>
        ...
        <main class="main">
          ...
          <div class="tabs-container">
            <div :class="['tab', { active: seccionActiva === 'monitor' }]" @click="seccionActiva = 'monitor'">Monitor en Vivo</div>
            <div :class="['tab', { active: seccionActiva === 'reservas' }]" @click="seccionActiva = 'reservas'">Reservaciones</div>
            <div :class="['tab', { active: seccionActiva === 'clientes' }]" @click="seccionActiva = 'clientes'">Clientes</div>
          </div>

          <div v-if="seccionActiva === 'monitor'">
            <div class="monitor-grid">
              <div v-for="cancha in monitorCanchas" :key="cancha.id" :class="['monitor-card', cancha.ocupada ? 'ocupada' : 'libre']">
          ...
          <div v-if="seccionActiva === 'reservas'">
            ...
            <div class="table-wrap">
              ...
              <div v-for="reserva in reservasFiltradas" :key="reserva.id" class="table-row reserva-row">
                ...
                <div class="row-date td-date">{{ formatearFecha(reserva.fecha) }}</div>
                <div class="row-time td-time">{{ formatearHora(reserva.hora_inicio) }} a {{ formatearHora(reserva.hora_fin) }}</div>
                ...
                <div class="mobile-card-meta">
                  <div class="mcm-group">
                    <span class="mcm-label">Cancha</span>
                    <span class="mcm-val td-court-sm">{{ reserva.cancha_nombre }}</span>
                  </div>
                  <div class="mcm-group">
                    <span class="mcm-label">Fecha</span>
                    <span class="mcm-val">{{ formatearFecha(reserva.fecha) }}</span>
                  </div>
                  <div class="mcm-group">
                    <span class="mcm-label">Horario</span>
                    <span class="mcm-val td-time-sm">{{ formatearHora(reserva.hora_inicio) }} — {{ formatearHora(reserva.hora_fin) }}</span>
                  </div>
                </div>
              ...
              <div class="section">
                ...
                <form @submit.prevent="registrarCliente" class="client-form">
                  ...
                  <div class="submit-area" style="margin-top:2.5rem">
                    <button type="submit" class="btn-main" :disabled="guardandoCliente">{{ guardandoCliente ? 'Registrando...' : 'Dar de alta cliente' }}</button>
                  </div>
                  <div v-if="mensajeCliente.texto" :class="['alert', mensajeCliente.error ? 'err' : 'ok']">{{ mensajeCliente.texto }}</div>
                </form>
              </div>
            </div>

            <div class="table-wrap" style="margin-top:3rem">
              <div class="desktop-head client-head">
              ...
              <div v-if="clientes.length === 0" class="empty">No hay clientes registrados en el sistema.</div>
              <div v-for="cliente in clientes" :key="cliente.id" class="table-row cliente-row">
                <div class="row-client">
                  <div class="td-name">{{ cliente.first_name }}</div>
                  <div class="td-phone desktop-only">{{ cliente.username }}</div>
                </div>
                <div class="row-username td-date">{{ cliente.username }}</div>
                <div class="row-tel" style="font-family:'Bebas Neue',sans-serif;font-size:16px;color:#A0A4B8;letter-spacing:0.04em">{{ cliente.telefono_display }}</div>
                <div class="row-status">
                ...
                <div class="mobile-card-meta">
                  <div class="mcm-group">
                    <span class="mcm-label">Usuario</span>
                    <span class="mcm-val">{{ cliente.username }}</span>
                  </div>
                  <div class="mcm-group">
                    <span class="mcm-label">Teléfono</span>
                    <span class="mcm-val td-time-sm">{{ cliente.telefono_display }}</span>
                  </div>
                </div>
                <div class="mobile-card-footer">
                  <span :class="['badge', cliente.is_active ? 'badge-ok' : 'badge-cancel']">{{ cliente.is_active ? 'ACTIVO' : 'PAUSADO' }}</span>
...
const confirmarReservaAdmin = async (id) => {
  ...
  } finally { procesandoId.value = null }
}

const cancelarReserva = async (id) => {
  if (!confirm('Confirmar la cancelacion de esta reserva')) return
  procesandoId.value = id
  try {
...
const registrarCliente = async () => {
  ...
  try {
    ...
    if (error.response?.data) {
      mensajeCliente.value = { texto: Object.values(error.response.data).flat().join(' | '), error: true }
    } else {
...
const cambiarEstadoCliente = async (cliente) => {
  ...
  try {
    ...
    if (error.response?.status === 401 || error.response?.status === 403) cerrarSesion()
  } finally { procesandoClienteId.value = null }
}

const eliminarCliente = async (id) => {
  if (!confirm('¿Estás seguro de eliminar permanentemente a este cliente? Se borrará su acceso.')) return
...
const iniciarSesion = async () => {
  ...
  try {
    ...
    if (error.response?.status === 401) mensajeError.value = 'Credenciales incorrectas'
    else if (error.response?.status === 403) mensajeError.value = 'Acceso denegado. Privilegios insuficientes.'
    else mensajeError.value = 'Error al conectar con el servidor.'
...
.logo-dot { width: 8px; height: 8px; background: #C2FF00; border-radius: 50%; flex-shrink: 0; }
...
.tabs-container { display: flex; margin-bottom: 2.5rem; border-bottom: 1px solid #2A2D3D; overflow-x: auto; -webkit-overflow-scrolling: touch; scrollbar-width: none; }
.tabs-container::-webkit-scrollbar { display: none; }
.tab { font-family: 'Syne', sans-serif; font-size: 11px; font-weight: 800; color: #5A5E70; letter-spacing: 0.18em; text-transform: uppercase; padding: 0.8rem 1.5rem; cursor: pointer; transition: all 0.2s; border-bottom: 2px solid transparent; margin-bottom: -1px; white-space: nowrap; flex-shrink: 0; }
...
.reservas-head { grid-template-columns: 2fr 1.6fr 1.2fr 1.2fr 1fr 1.4fr; }
...
.table-row { border-bottom: 1px solid #2A2D3D; transition: background 0.15s; }
.table-row:last-child { border-bottom: none; }
.table-row:hover { background: #1A1D28; }

.reserva-row { display: grid; grid-template-columns: 2fr 1.6fr 1.2fr 1.2fr 1fr 1.4fr; padding: 1rem 1.5rem; align-items: center; }
.cliente-row { display: grid; grid-template-columns: 2fr 1.5fr 2fr 1fr 1.5fr; padding: 1rem 1.5rem; align-items: center; }
...
@media (max-width: 900px) {
  ...
  .login-page .sidebar { width: 100%; position: static; height: auto; }
  .login-page .sidebar .sidebar-inner { padding: 2rem 1.5rem 1.5rem; }
  .login-page .sidebar .sidebar-title { font-size: 36px; }
  .login-page .sidebar .sidebar-sub { margin-bottom: 0; }
  .login-page .sidebar .sidebar-info { display: none; }
  .login-page .sidebar .sidebar-badge { margin-top: 1.5rem; }
  .login-main { padding: 2rem 1.25rem 3rem; align-items: flex-start; }
  ...
  .sidebar.sidebar-open { left: 0; box-shadow: 6px 0 40px rgba(0,0,0,0.6); }
  .sidebar-inner { padding: 2rem 1.5rem; }

  .sidebar-overlay { display: block; position: fixed; inset: 0; background: rgba(0,0,0,0.6); z-index: 999; }
  ...
  .hamburger { display: flex; flex-direction: column; gap: 5px; background: transparent; border: none; cursor: pointer; padding: 4px; }
  .hamburger span { display: block; width: 22px; height: 2px; background: #C2FF00; border-radius: 2px; }
  .btn-logout-mobile { display: block; background: transparent; border: 1px solid rgba(255,80,80,0.25); border-radius: 6px; padding: 0.4rem 0.75rem; font-family: 'Syne', sans-serif; font-size: 9px; font-weight: 800; color: #FF6060; letter-spacing: 0.12em; text-transform: uppercase; cursor: pointer; }

  .main { padding: 1.5rem 1.25rem 3rem; }
  .main-top { display: none; }
  .welcome-banner { margin-bottom: 1.5rem; padding-bottom: 1.25rem; }
  .welcome-banner h1 { font-size: 28px; }
  .welcome-banner p { font-size: 13px; }

  .tabs-container { margin-bottom: 1.5rem; }
  .tab { padding: 0.65rem 1rem; font-size: 9px; letter-spacing: 0.1em; }

  .monitor-grid { grid-template-columns: 1fr; gap: 1rem; }
  ...
  .row-court, .row-date, .row-time, .row-status, .row-actions,
  .row-username, .row-tel { display: none; }
  ...
  .mcm-group { display: flex; flex-direction: column; gap: 2px; }
  .mcm-label { font-family: 'Syne', sans-serif; font-size: 8px; font-weight: 700; color: #8A8E9B; letter-spacing: 0.15em; text-transform: uppercase; }
